...
│   └── e1_17/        DPI 感知與混合 DPI（E1-17）：多螢幕各自的邏輯縮放（scale_factor）以具名螢幕
│                     ScreenId 與無單位比值表達，混合 DPI 各螢幕獨立。無絕對座標 / 數字 z-order（NFR-02）；
│                     相位 1 null 後端回預設縮放 1.0、未知螢幕保守回 1.0。
...
├── gates/            治理閘門自身的測試（Python，由 G0 直接跑，不經 CTest）
...
scripts/              治理工具（plan / scope_check / backend_guard / stage_check / status_check / workflow_lint / run_python_tests / halt_gate）

> `tests/` 底下**任何位置**的 `test_*.py` 都會被 G0 `run_python_tests.py` 收到
> （它自己走檔案樹；`unittest discover` 對非套件子目錄會跑 0 個測試然後回綠）。
```

...
- 日期：2026-08-03 | 來源：`CHG-20260803-10`（修 [[K-003]] 的當下發現）| 狀態：**已釘住，未對齊**
...
對齊要改動承重單元 E1-24 的行為並牽動 16 個單元的測試，屬**獨立的方向決策**，
不該夾帶在「修契約鴻溝」那張 CHG 裡。為了讓分歧可見且被追蹤，兩邊各留一條測試釘住現況：
...
對齊之後，這兩條應被刪除，前置條件放回共用契約
（`tests/contract/kernel_backend_contract.hpp` 已標註該位置）。

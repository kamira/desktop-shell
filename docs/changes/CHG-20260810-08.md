...
Accepted — self-verified（medium：需求分析 + 結構設計 + 寫入 `units.json`，未動程式碼）。

**confirm 停點紀錄（2026-08-10）**：設計圖與 6 個單元的切分已呈給使用者，
**使用者放行並指定 W1-07 先做**。本檔隨即完成其產出——6 個單元已寫入 `units.json`
（總數 187 → **193**）。

> 本檔曾以 `Paused` 進 main（`56b0152` / #215），那是刻意的：
> `CHG-20260810-03` 的 G7 會擋下 `Proposed` 的 CHG，而 `Paused` 是它明訂的合法 WIP。
> 一份**刻意擱置、等人決定**的設計紀錄應該進得了 main，否則它只會留在某個人的機器上。
> 這是那個逃生口的第一個真實用例。

## Verification

### 本 CHG 自身（寫入 units.json）

```bash
python3 scripts/stage_check.py && python3 scripts/run_python_tests.py
```

```
stage_check OK — 193 個單元無階段違規
run_python_tests OK（7 支檔、111 個測試，跳過 1）
```

`CHG-20260810-06` 新加的三條真實資料回歸測試**當場派上用場**，
驗證了新單元的 `depends_on` 都指向存在的 id、每個 `branch` slug 都推得回 id、
193 個 `brief()` 都渲染得出來（`BRIEF.format` 少一個欄位會在**派工當下**才炸）。

### 順帶修掉一個會讓派工出錯的問題：`state.json` 漏了 7 個單元

寫完單元後跑 `plan.py next` 驗證，它把 **W1-03 / W1-04 當成待派工**——
那兩個 2026-08-03 就合併了。而要做的 W1-07 根本沒出現在清單上。

根因：`docs/backlog/state.json` 只記了 180 個完成單元，
漏掉 **W1-03 / W1-04 / W1-05 / W1-06 / H1-03 / H1-04 / H1-05** 共 7 個。
`HANDOFF` §0-E 記載的「state.json 已重建」是用 **CHG 檔名**反推的，
而那批單元的 CHG 檔名不帶 slug（`CHG-20260803-09.md` 而非 `-w1_03`），
檔名反推法一個都抓不到。

這不是紀錄好看與否的問題——**AGENTS.md §3 說工作單元只能來自 `plan.py next`**，
派工來源給錯答案，就會有人去重做已經做完的東西。

改用 **commit 訊息**反推（commit 一定帶單元代號，見 §4 的 PR 慣例）重建：

```
state.json done: 180 → 187
plan.py status: 總計 193 單元｜已完成 187｜進行中 0
plan.py next:   W1-07（w17）、H1-06（w18）  ← 正確
```

### ⚠ W1-07 會停在 G5，需要指定 V1

`W1-07` 是 **medium**，G5 對 medium 單元要求
`docs/acceptance/ACC-*-w1_07.md` 存在、`Result: pass`、
且 **`Verifier` ≠ `Implemented by`**（AGENTS.md §4：實作者不自我驗收）。

I1 不能自我驗收，本次執行環境也不允許自行派 subagent 當 V1。
**這不是可以繞過的閘門**——它擋的正是「實作者說自己做對了」。
需使用者指定驗收者，或明示授權派 V1。H1-06 / H1-07 / H1-09 同為 medium，同樣適用。

### 各單元的驗收（先行約定，避免事後遷就結果）

...
**沒有用 `unittest discover`**：那一行對本 repo **一個測試都收不到**
（discover 只遞迴進有 `__init__.py` 的套件，`tests/` 的子目錄都不是）。

> 更正：初版還說「而且會安靜地回綠」，並把 exit 0 寫死成斷言。那只在 Python ≤3.11
> 成立（本機 3.11.9），3.12 起回 exit 5（CI 3.12.13）。**G0 上線第一次執行就抓到
> 作者自己把版本相依行為當成通則**。已拆成版本無關／相依兩段斷言。
